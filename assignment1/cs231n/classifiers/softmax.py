...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    dW += np.matmul(np.expand_dims(X[i], axis = 1), np.expand_dims(scores, axis = 0)) / float(scores_sum)
    dW[:, y[i]] -= X[i]
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  dW = - np.matmul(np.transpose(X), one_hot)
  dW += np.matmul(np.transpose(X), scores / scores_sum)
