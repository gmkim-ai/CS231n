...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ReLU, ReLU_cache = relu_forward(L2)
        L3, L3_cache = affine_forward(ReLU, W3, b3)
        ...
        dx, dw, db = affine_backward(dl, L3_cache)
        grads['W3'] = dw + self.reg * W3
        grads['b3'] = db

        dReLU = relu_backward(dx, ReLU_cache)
        dx, dw, db = affine_backward(dReLU, L2_cache)
        grads['W2'] = dw + self.reg * W2
        grads['b2'] = db
 
        dx, dw, db = conv_relu_pool_backward(dx, L1_cache)
