...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_mean = x - mean
        var = np.sum(x_mean ** 2, axis = 0) / float(N)
        std = np.sqrt(var + eps)
        inv_std = 1 / std
        norm = x_mean * inv_std
        out = gamma * x_mean * inv_std + beta

        cache = (norm, inv_std, std, var, x_mean, eps, gamma)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    norm, inv_std, std, var, x_mean, eps, gamma = cache
    N, D = dout.shape

    dbeta = np.sum(dout, axis = 0)
    dgamma = np.sum(dout * norm, axis = 0)
    dnorm = dout * gamma
    dinv_std = np.sum(dnorm * x_mean, axis = 0)
    dx_mean = dnorm * inv_std
    dstd = - dinv_std / (std ** 2)
    dvar = 0.5 * dstd / std
    dx_mean += 2 * x_mean * dvar * np.ones((N, D)) / float(N)
    dmean = - np.sum(dx_mean, axis = 0)

    dx = dx_mean + dmean * np.ones((N, D)) / float(N) 
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    dbeta = np.sum(dout, axis = 0)
    dgamma = np.sum(dout * norm, axis = 0)
    dx = gamma * inv_std * (N * dout - np.sum(dout, axis = 0) \
            - x_mean * (var + eps) ** -1 * np.sum(dout * x_mean, axis = 0)) / float(N)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mask = np.where(np.random.random_sample(x.shape) >= p, 1.0, 0)
        mask *= 1 / (1 - p)
        ...
        for f in range(F):
            for j in range(out_H): 
                for i in range(out_W):
                    out[n, f, j, i] = np.sum(w[f, ] * pad_x[n, :, j*stride:j*stride+HH, i*stride:i*stride+WW]) + b[f]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
        ...
        for f in range(F):
            for j in range(H_out):
                for i in range(W_out):
                    dw[f, ] += dout[n, f, j, i] * pad_x[n, :, j*stride:j*stride+HH, i*stride:i*stride+WW]
                    dx[n, :, j*stride:j*stride+HH,i*stride:i*stride+WW] += dout[n, f, j, i] * w[f, ]
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for j in range(out_H):
                for i in range(out_W): 
                    index = np.argmax(x[n, c, j*stride:j*stride+pool_height, i*stride:i*stride+pool_width])
                    max_coord = np.unravel_index(index, [pool_height, pool_width])
